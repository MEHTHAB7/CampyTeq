...
class IsPrincipal(permissions.BasePermission):
    """Allows access to Principal or Super Admin."""
    def has_permission(self, request, view):
        return bool(
            request.user and request.user.is_authenticated and
            request.user.role in ['SUPER_ADMIN', 'PRINCIPAL']
        )


class IsAdminOrPrincipal(permissions.BasePermission):
    """Allows access to Super Admin, Principal, or Management."""
    def has_permission(self, request, view):
        return bool(
            request.user and request.user.is_authenticated and
            request.user.role in ['SUPER_ADMIN', 'PRINCIPAL', 'MANAGEMENT']
        )
...
class IsTenantMember(permissions.BasePermission):
    ...
    1. Authenticated user must belong to a college (unless SUPER_ADMIN).
